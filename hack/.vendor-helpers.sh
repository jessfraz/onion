...
PROJECT=github.com/jfrazelle/onion

# Downloads dependencies into vendor/ directory
mkdir -p vendor

if ! go list ${PROJECT} &> /dev/null; then
	rm -rf .gopath
	mkdir -p .gopath/src/github.com/jfrazelle
...
clean() {
	...
	local dockerPlatforms=( ${DOCKER_ENGINE_OSARCH:="linux/amd64"} )
	local dockerBuildTags=""
